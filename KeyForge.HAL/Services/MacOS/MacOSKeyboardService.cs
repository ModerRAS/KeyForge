...
/// 这是简化实现，专注于核心功能
/// </summary>
public class MacOSKeyboardService : IKeyboardService
{
    private readonly ILogger<MacOSKeyboardService> _logger;
    ...
    public MacOSKeyboardService(ILogger<MacOSKeyboardService> logger)
    {
    ...
    public async Task<bool> KeyDownAsync(KeyCode key)
    {
        try
        {
            ...
            {
                // 简化实现 - 记录日志
                _logger.LogDebug("MacOS key down: {Key}", key);
    ...
    public async Task<bool> KeyUpAsync(KeyCode key)
    {
        try
        {
            ...
            {
                // 简化实现 - 记录日志
                _logger.LogDebug("MacOS key up: {Key}", key);
    ...
    public async Task<bool> TypeTextAsync(string text, int delay = 50)
    {
        try
        {
            ...
            _logger.LogDebug("MacOS typing text: {Text} with delay: {Delay}ms", text, delay);
    ...
    public async Task<bool> SendHotkeyAsync(KeyCode[] modifiers, KeyCode key)
    {
        try
        {
            _logger.LogDebug("MacOS sending hotkey: {Modifiers} + {Key}", 
    ...
    public KeyState GetKeyState(KeyCode key)
    {
        ...
        {
            // 简化实现 - 返回默认状态
            _logger.LogDebug("MacOS get key state: {Key}", key);
            return KeyState.Up;
        }
        catch (Exception ex)
        {
