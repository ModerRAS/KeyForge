...
/// 这是简化实现，专注于核心功能
/// </summary>
public class MacOSGlobalHotkeyService : IGlobalHotkeyService
{
    private readonly ILogger<MacOSGlobalHotkeyService> _logger;
    ...
    public MacOSGlobalHotkeyService(ILogger<MacOSGlobalHotkeyService> logger)
    {
    ...
    public async Task<bool> RegisterHotkeyAsync(int id, KeyCode[] modifiers, KeyCode key, Action<HotkeyEventArgs> callback)
    {
        try
        {
            lock (_lock)
            {
                ...
                _logger.LogInformation("MacOS hotkey registered: {Id}, {Description}", id, hotkeyInfo.Description);
    ...
    public async Task<bool> UnregisterHotkeyAsync(int id)
    {
        try
        {
            lock (_lock)
            {
                ...
                _logger.LogInformation("MacOS hotkey unregistered: {Id}", id);
    ...
    public async Task<bool> SuspendAllHotkeysAsync()
    {
        try
        {
            lock (_lock)
            {
                _logger.LogInformation("MacOS all hotkeys suspended");
                return true;
            }
        }
        catch (Exception ex)
    ...
    public async Task<bool> ResumeAllHotkeysAsync()
    {
        try
        {
            lock (_lock)
            {
                _logger.LogInformation("MacOS all hotkeys resumed");
                return true;
            }
        }
        catch (Exception ex)
