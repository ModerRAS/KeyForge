...
/// 这是简化实现，专注于核心功能
/// </summary>
public class LinuxPerformanceMonitor : IPerformanceMonitor
{
    private readonly ILogger<LinuxPerformanceMonitor> _logger;
    ...
    public LinuxPerformanceMonitor(ILogger<LinuxPerformanceMonitor> logger)
    {
    ...
    public async Task CollectMetricsAsync()
    {
        try
        {
            ...
            _logger.LogDebug("Linux performance metrics collected: CPU={Cpu}%, Memory={Memory}MB, Disk={Disk}%, Network={Network}%",
    ...
    public void RecordMetric(string name, double value, Dictionary<string, string>? tags = null)
    {
        try
        {
            ...
            _logger.LogDebug("Linux custom metric recorded: {Name}={Value}", name, value);
        }
        catch (Exception ex)
        {
    ...
    public async Task<BenchmarkResult> RunBenchmarkAsync(BenchmarkRequest request)
    {
        try
        {
            _logger.LogInformation("Linux running benchmark: {TestName}", request.TestName);
            ...
            _logger.LogInformation("Linux benchmark completed: {TestName}, Average: {Average}ms, Min: {Min}ms, Max: {Max}ms",
    ...
    public async Task<PerformanceReport> GenerateReportAsync(DateTimeRange range)
    {
        try
        {
            _logger.LogInformation("Linux generating performance report for range: {Start} to {End}", range.Start, range.End);
            ...
            _logger.LogInformation("Linux performance report generated with {Count} metrics", metrics.Count);
            return report;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> StartMonitoringAsync(int interval = 1000)
    {
        try
        {
            if (_isMonitoring)
            {
                _logger.LogWarning("Linux performance monitoring is already running");
                return false;
            }
            ...
            _logger.LogInformation("Linux performance monitoring started with interval: {Interval}ms", interval);
            return true;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> StopMonitoringAsync()
    {
        try
        {
            if (!_isMonitoring)
            {
                _logger.LogWarning("Linux performance monitoring is not running");
                return false;
            }
            ...
            _isMonitoring = false;
            _logger.LogInformation("Linux performance monitoring stopped");
            return true;
        }
        catch (Exception ex)
        {
    ...
    public int CleanupHistoricalData(DateTime olderThan)
    {
        try
        {
            lock (_metricsLock)
            {
                ...
                _logger.LogDebug("Linux cleaned {Count} historical metrics older than {Date}", cleanedCount, olderThan);
                return cleanedCount;
            }
        }
        catch (Exception ex)
    ...
    private async Task TriggerAlertAsync(AlertLevel level, string message, Dictionary<string, object> details)
    {
        try
        {
            ...
            _logger.LogWarning("Linux performance alert triggered: {Level} - {Message}", level, message);
        }
        catch (Exception ex)
        {
