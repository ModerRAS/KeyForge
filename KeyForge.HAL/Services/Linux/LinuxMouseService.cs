...
{
    private readonly ILogger<LinuxMouseService> _logger;
    ...
    public LinuxMouseService(ILogger<LinuxMouseService> logger)
    {
    ...
    public async Task<bool> MoveToAsync(int x, int y)
    {
        try
        {
            ...
            {
                // 简化实现 - 记录日志
                _logger.LogDebug("Linux mouse moved to: ({X}, {Y})", x, y);
    ...
    public async Task<bool> LeftButtonDownAsync()
    {
        try
        {
            lock (_lock)
            {
                _logger.LogDebug("Linux left mouse button down");
    ...
    public async Task<bool> LeftButtonUpAsync()
    {
        try
        {
            lock (_lock)
            {
                _logger.LogDebug("Linux left mouse button up");
    ...
    public async Task<bool> RightClickAsync()
    {
        try
        {
            lock (_lock)
            {
                _logger.LogDebug("Linux right mouse button click");
    ...
    public async Task<bool> MiddleClickAsync()
    {
        try
        {
            lock (_lock)
            {
                _logger.LogDebug("Linux middle mouse button click");
    ...
    public async Task<bool> ScrollAsync(int delta)
    {
        try
        {
            lock (_lock)
            {
                _logger.LogDebug("Linux mouse scrolled: {Delta}", delta);
    ...
    public Point GetPosition()
    {
        ...
        {
            // 简化实现 - 返回默认位置
            _logger.LogDebug("Linux get mouse position");
    ...
    public MouseButtonState GetButtonState(MouseButton button)
    {
        ...
        {
            // 简化实现 - 返回默认状态
            _logger.LogDebug("Linux get mouse button state: {Button}", button);
