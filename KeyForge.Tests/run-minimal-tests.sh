...
if dotnet test "$SCRIPT_PATH/KeyForge.Tests.Minimal.csproj" \
    ...
    # 尝试打开HTML报告
    if [ -f "$RESULTS_PATH/Reports/test-report.html" ]; then
        log_info "HTML报告已生成: $RESULTS_PATH/Reports/test-report.html"
    fi
    ...
    log_error "最小测试运行失败，退出代码: $EXIT_CODE"
    log_info "耗时: $DURATION 秒"
